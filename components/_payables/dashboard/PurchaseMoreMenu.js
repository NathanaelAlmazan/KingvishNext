...
export default function UserMoreMenu(props) {
  ...
  const cancelOrder = async () => {
    try {
      await axios.get(`${baseURL}/payables/cancel/${id}`, config);
      setOpenSuccess(`Sucess: Purchase ${id} was canceled successfully.`);
      setIsOpen(false);
  ...
  return (
    <>
      ...
      >
        ...
        {!isPaid && (
          <MenuItem onClick={e => onRouterClick(e, `/purchase/payment/${id}`)} sx={{ color: 'text.secondary' }}>
            <ListItemIcon>
              < PaymentsIcon />
            </ListItemIcon>
            <ListItemText primary="Record Payment" primaryTypographyProps={{ variant: 'body2' }} />
          </MenuItem>
        )}
        ...
        {Boolean(!delivered && ExecutivePosition.includes(position)) && (
          <MenuItem onClick={cancelOrder} sx={{ color: 'text.secondary' }}>
            ...
            <ListItemText primary="Cancel Purchase" primaryTypographyProps={{ variant: 'body2' }} />
          </MenuItem>
        )}
