...
export default function UserMoreMenu(props) {
  ...
  const deleteOrder = async () => {
    try {
      await axios.get(`${baseURL}/sales/delete-order/${id}`, config);
      setIsOpen(false);
      refreshData();
  ...
  const restoreOrder = async () => {
    try {
      await axios.get(`${baseURL}/sales/restore-order/${id}`, config);
      setIsOpen(false);
      refreshData();
      ...
      >
        ...
        {purpose === "delivery" && (
          <MenuItem onClick={deliverOrder} sx={{ color: 'text.secondary' }}>
            ...
            <ListItemText primary="Delivered" primaryTypographyProps={{ variant: 'body2' }} />
          </MenuItem>
        )}
