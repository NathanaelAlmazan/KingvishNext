...
export default function DeliverOrders(props) {
    ...
    const deliverOrders = async () => {
        ...
        try {
            for (let i = 0; i < selected.length; i++) {
                await axios.get(`${baseURL}/payables/delivered/${selected[i]}`, config);
            }
          
          setOpenSuccess(`Sucess: ${selected.length} orders arrived successfully.`);
          setSelected([]);
          refetchData();
    ...
    const refetchData = async () => {
        ...
        try {
            ...
            forceUpdate();
            setOrders(deliveryOrders.purchasedOrdersDelivery);
...
export async function getServerSideProps(ctx) {
    ...
    try {
        ...
        return {
            props: { deliveryOrders: deliveryOrders.purchasedOrdersDelivery }
        }
