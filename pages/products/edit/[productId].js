...
export default function EditProduct(props) {
    const { currUser, productData } = props;
    ...
    useEffect(() => {
        setProduct(product => ({ ...product,  
            ...
            barCode: productData.bar_code !== null ? productData.bar_code : "",
       ...
       if (productData.details.length > 0) {
            ...
            productData.details.forEach(detail => {
                if (detail.unit !== null) {
                    productMeasure.push(detail);
                } else {
                    productDesc.push(detail);
                }
            });

            setMeasurements(state => productMeasure);
            setDescriptions(state => productDesc);
            setImageName(productData.image_name);
       }
    }, [productData]);
        ...
        if (measurements.length === 0 && descriptions.length === 0) {
            ...
            try {
                ...
                const response = await axios({
                    ...
                    data: {
                    query: `
                        mutation UpdateProduct ($ID: Int!, $Name: String, $Code: String, $Category: Int, $Price: Float, $inStock: Int) {
                            updateProduct (id: $ID, name: $Name, bar_code: $Code, category_id: $Category, price: $Price, stocks: $inStock) {
                                id
                                name
                            }
                        }
                    ...
                    variables: {
                        ...
                        Category: category,
                        Price: price,
                        inStock: stocks !== "" ? stocks : null
                    }
                    }
                });

                const updatedProduct = response.data.data;
                const productId = updatedProduct.updateProduct.id;
                ...
                for (let x=0; x < ProdDetails.length; x++) {
                    ...
                    await axios({
                        ...
                        data: {
                        query: `
                            mutation UpdateDetailToProduct ($Product: Int!, $Type: String!, $nValue: Float, $tValue: String, $Unit: String) {
        ...
        <Container>
            <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
                ...
                <Button
                    color="error"
                    variant="contained"
                    onClick={e => handleRedirect("/products/profile/" + productData.id)}
                    startIcon={<CloseIcon />}
                >
                    Cancel
                </Button>
